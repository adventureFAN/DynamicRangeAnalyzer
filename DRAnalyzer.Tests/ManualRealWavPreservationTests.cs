...
public sealed class ManualRealWavPreservationTests
{
    [Fact]
    public void ReferenceCopy_WriteAndRemove_PreservesRealWavFile()
    {
        var albumDirectory =
        ...
        Assert.False(
            ...
            "DRANALYZER_REFERENCE_WAV_DIR ist nicht gesetzt.");

        Assert.True(
            Directory.Exists(albumDirectory),
            $"WAV-Referenzordner fehlt: {albumDirectory}");

        var originalPath =
            FindFirstSupportedWave(albumDirectory!);

        Assert.False(
            string.IsNullOrWhiteSpace(originalPath),
            "Im Referenzordner wurde keine vom aktuellen WAV-Writer unterstützte .wav-Datei gefunden.");
        ...
        try
        {
            ...
            WavDynamicRangeTagWriter.Remove(copyPath);
                ...
                beforeSnapshot.Id3Chunk is null)
            {
                Assert.True(
                    ...
                    "Write -> Remove hat eine ursprünglich ID3-/DR-taglose WAV-Datei nicht bytegenau wiederhergestellt.");
            }
            ...
            Console.WriteLine(
                $"WAV realfile: {Path.GetFileName(originalPath)}");

            Console.WriteLine(
                $"Channels: {beforeAnalysis.Channels}");

            Console.WriteLine(
                $"PCM data bytes preserved: {dataChunk.PayloadLength}");

            Console.WriteLine(
                $"Foreign RIFF chunks preserved: {beforeSnapshot.Chunks.Count(chunk => !chunk.IsId3)}");
    ...
    private static string? FindFirstSupportedWave(
    ...
    private static WavSnapshot ReadSnapshot(
        byte[] bytes)
    {
        if (bytes.Length < 12)
            throw new InvalidDataException("WAV-Datei ist zu kurz.");

        if (!bytes.AsSpan(0, 4).SequenceEqual(Encoding.ASCII.GetBytes("RIFF")) ||
            !bytes.AsSpan(8, 4).SequenceEqual(Encoding.ASCII.GetBytes("WAVE")))
        {
            throw new NotSupportedException("Nur klassisches RIFF/WAVE wird für diesen Test akzeptiert.");
        }
        ...
        if ((long)declaredSize + 8 != bytes.Length)
            throw new InvalidDataException("RIFF-Größe stimmt nicht mit der Dateigröße überein.");

        var chunks =
            new List<RiffChunkSnapshot>();

        RiffChunkSnapshot? id3Chunk = null;
        ...
        while (offset < bytes.Length)
        {
            if (bytes.Length - offset < 8)
                throw new InvalidDataException("Abgeschnittener RIFF-Chunk-Header.");
            ...
            if (offset + totalLength > bytes.Length)
                throw new InvalidDataException($"RIFF-Chunk '{id}' reicht über das Dateiende hinaus.");
        ...
        if (!sawFmt || !sawData)
            throw new InvalidDataException("WAV benötigt fmt- und data-Chunk.");

        return new WavSnapshot(
            chunks,
            id3Chunk,
            id3Frames);
    }
        ...
        byte[] payload)
    {
        ...
        if (10 + bodyLength != payload.Length)
            throw new InvalidDataException("ID3v2-Größe passt nicht zum WAV-ID3-Chunk.");
        ...
        WavSnapshot after)
    {
             ...
             index++)
        {
            ...
            Assert.True(
                ...
                $"RIFF-Chunk '{beforeForeign[index].Id}' an Position {index} wurde verändert oder umsortiert.");
        }
    }
    ...
    private sealed record WavSnapshot(
        IReadOnlyList<RiffChunkSnapshot> Chunks,
        RiffChunkSnapshot? Id3Chunk,
        IReadOnlyList<Id3FrameSnapshot> Id3Frames);

    private sealed record RiffChunkSnapshot(
        string Id,
        int PayloadLength,
        byte[] RawBytes,
        bool IsId3);
