...
public static class WavPackDynamicRangeTagWriter
{
    ...
    private static readonly byte[] WavPackMarker = Encoding.ASCII.GetBytes("wvpk");
    private static readonly byte[] Id3v1Marker = Encoding.ASCII.GetBytes("TAG");
    private const int WavPackHeaderLength = 32;
    private const int Id3v1Length = 128;
    private const ushort MinimumWavPackVersion = 0x0402;
    private const ushort MaximumWavPackVersion = 0x0410;
    ...
    public static void Write(string filePath, int trackDynamicRange, int? albumDynamicRange)
    {
        ...
        if (!File.Exists(filePath))
            throw new FileNotFoundException("Die WavPack-Datei wurde nicht gefunden.", filePath);
        ...
        Action<string> validateTemp)
    {
        ...
        try
        {
            WriteModifiedCopy(fullPath, tempPath, source.WavPackDataLength, replacementTag, source.TrailingId3v1);
        ...
        byte[]? trailingId3v1)
    {
        ...
        if (trailingId3v1 is not null)
            output.Write(trailingId3v1);
        ...
        int? albumDynamicRange)
    {
        ...
        ValidatePrefixPreserved(
            sourcePath,
            source.WavPackDataLength,
            destinationPath,
            destination.WavPackDataLength);

        ValidateTrailingId3v1Preserved(source.TrailingId3v1, destination.TrailingId3v1);
        ...
        long destinationLength)
    {
        if (sourceLength != destinationLength)
            throw new InvalidDataException("Der WavPack-Datenbereich wurde verschoben oder in der Länge verändert.");
        ...
        while (remaining > 0)
        {
            ...
            if (!sourceBuffer.AsSpan(0, requested).SequenceEqual(destinationBuffer.AsSpan(0, requested)))
                throw new InvalidDataException("Der WavPack-Datenbereich wurde verändert.");

            remaining -= requested;
        }
    }

    private static void ValidateTrailingId3v1Preserved(byte[]? source, byte[]? destination)
    {
        if (source is null && destination is null)
            return;

        if (source is null || destination is null || !source.AsSpan().SequenceEqual(destination))
            throw new InvalidDataException("Ein vorhandener ID3v1-Trailer wurde verändert oder verschoben.");
    }

    private static ParsedWavPackFile ReadFile(string filePath)
    {
        using var stream = new FileStream(filePath, FileMode.Open, FileAccess.Read, FileShare.Read);

        if (stream.Length < WavPackHeaderLength)
            throw new InvalidDataException("Die Datei ist zu kurz für WavPack.");

        var trailingId3v1 = TryReadTrailingId3v1(stream);
        var effectiveEnd = stream.Length - (trailingId3v1?.Length ?? 0);

        if (effectiveEnd < WavPackHeaderLength)
            throw new InvalidDataException("Die WavPack-Datei ist zu kurz.");

        ParsedApeTag? tag = null;
        long wavPackDataLength = effectiveEnd;

        if (effectiveEnd >= DescriptorLength)
        {
            stream.Position = effectiveEnd - DescriptorLength;
            ...
            if (footerRaw.AsSpan(0, 8).SequenceEqual(ApeTagMarker))
            {
                ...
                // Historical real-world quirk: bit 30 may claim there is no footer even
                // though a valid footer is physically present at the expected tag end.
                // The physical structure is authoritative and the legacy bit is preserved.
                ...
                if (totalTagLength > effectiveEnd - WavPackHeaderLength)
                    throw new InvalidDataException("Der APEv2-Tag ragt über den WavPack-Datenbereich hinaus.");
    ...
    private static byte[]? TryReadTrailingId3v1(FileStream stream)
    {
        ...
        var trailer = new byte[Id3v1Length];
        ReadExactly(stream, trailer);

        return trailer.AsSpan(0, 3).SequenceEqual(Id3v1Marker) ? trailer : null;
    }

    private static void ValidateWavPackBlocks(FileStream stream, long dataLength)
    {
        long offset = 0;
        var header = new byte[WavPackHeaderLength];

        while (offset < dataLength)
        {
            if (dataLength - offset < WavPackHeaderLength)
                throw new InvalidDataException("Der WavPack-Datenbereich endet mitten in einem Blockheader.");

            stream.Position = offset;
            ReadExactly(stream, header);

            if (!header.AsSpan(0, 4).SequenceEqual(WavPackMarker))
                throw new InvalidDataException("Der WavPack-Datenbereich enthält einen ungültigen Blockmarker.");

            var blockSizeMinusEight = BinaryPrimitives.ReadUInt32LittleEndian(header.AsSpan(4, 4));
            var blockLength = checked((long)blockSizeMinusEight + 8);
            var version = BinaryPrimitives.ReadUInt16LittleEndian(header.AsSpan(8, 2));

            if (blockLength < WavPackHeaderLength)
                throw new InvalidDataException("Ein WavPack-Block meldet eine ungültige Größe.");

            if (version < MinimumWavPackVersion || version > MaximumWavPackVersion)
                throw new NotSupportedException($"Die WavPack-Streamversion 0x{version:X4} wird nicht unterstützt.");
    ...
    private sealed record ParsedWavPackFile(long WavPackDataLength, ParsedApeTag? Tag, byte[]? TrailingId3v1);
