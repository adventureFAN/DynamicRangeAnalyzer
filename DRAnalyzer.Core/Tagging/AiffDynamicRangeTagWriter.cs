...
public static class AiffDynamicRangeTagWriter
{
    ...
    private static readonly byte[] FormMarker = Encoding.ASCII.GetBytes("FORM");
    private static readonly byte[] AiffMarker = Encoding.ASCII.GetBytes("AIFF");
    ...
    public static void Write(string filePath, int trackDynamicRange, int? albumDynamicRange)
    {
        ...
        if (!File.Exists(filePath))
            throw new FileNotFoundException("Die AIFF-Datei wurde nicht gefunden.", filePath);
        ...
        var editedPayload = BuildId3Payload(targetVersion, editedBody, sourceId3.TrailingChunkPadding);
    ...
    public static void Remove(string filePath)
    {
        ...
        var editedPayload = BuildId3Payload(sourceId3.Version!.Value, editedBody, sourceId3.TrailingChunkPadding);
    ...
    private static byte[] BuildId3Payload(int version, byte[] body, byte[] trailingChunkPadding)
    {
        ArgumentNullException.ThrowIfNull(trailingChunkPadding);

        var payload = new byte[Id3HeaderLength + body.Length + trailingChunkPadding.Length];
        ...
        trailingChunkPadding.CopyTo(payload, Id3HeaderLength + body.Length);
        return payload;
    }
        ...
        bool removeId3Chunk)
    {
        ...
        output.Write(FormMarker);
        output.Write(new byte[4]); // FORM size is patched after all chunks are written.
        output.Write(AiffMarker);
        ...
        var formSize = output.Length - 8;
        if (formSize > uint.MaxValue)
            throw new NotSupportedException("AIFF-Dateien, die nach dem Tagging die AIFF/IFF-4-GB-Grenze überschreiten, werden derzeit nicht unterstützt.");

        output.Position = 4;
        Span<byte> formSizeBytes = stackalloc byte[4];
        BinaryPrimitives.WriteUInt32BigEndian(formSizeBytes, checked((uint)formSize));
        output.Write(formSizeBytes);
    ...
    private static void CopyRange(Stream input, Stream output, long offset, long length)
    {
        ...
        while (remaining > 0)
        {
            ...
            if (read == 0)
                throw new EndOfStreamException("Unerwartetes Dateiende beim Kopieren eines IFF-Chunks.");
    ...
    private static void ValidateWrittenCopy(
        ...
        int? albumDynamicRange)
    {
        var destination = ReadAiff(destinationPath);
        var destinationId3 = destination.Id3Tag
            ?? throw new InvalidDataException("Der geschriebene AIFF enthält keinen ID3v2-Chunk.");

        if (destinationId3.Version != targetVersion)
            throw new InvalidDataException("Der geschriebene AIFF enthält nicht die erwartete ID3v2-Version.");
    ...
    private static void ValidateForeignId3FramesPreserved(ParsedId3Tag? source, ParsedId3Tag destination)
    {
        ...
        AssertRawFrameSequenceEqual(sourceFrames, destinationFrames,
            "Fremde ID3v2-Frames im AIFF wurden verändert oder umsortiert.");
    }
        ...
        ParsedAiff destination)
    {
        ...
        if (sourceChunks.Length != destinationChunks.Length)
            throw new InvalidDataException("Fremde IFF-Chunks wurden hinzugefügt oder entfernt.");
        ...
        for (var i = 0; i < sourceChunks.Length; i++)
        {
            ...
            {
                throw new InvalidDataException("Fremde IFF-Chunks wurden verändert oder umsortiert.");
            }
            ...
            AssertRangesEqual(
                ...
                "Ein fremder IFF-Chunk wurde byteweise verändert.");
        }
    }
    ...
    private static ParsedAiff ReadAiff(string filePath)
    {
        using var stream = new FileStream(filePath, FileMode.Open, FileAccess.Read, FileShare.Read);

        if (stream.Length < 12)
            throw new InvalidDataException("Die AIFF-Datei ist zu kurz für einen FORM/AIFF-Header.");

        Span<byte> header = stackalloc byte[12];
        ReadExactly(stream, header);

        var container = Encoding.ASCII.GetString(header[..4]);
        if (!string.Equals(container, "FORM", StringComparison.Ordinal))
            throw new InvalidDataException("Die Datei besitzt keinen FORM/IFF-Header.");

        var formType = Encoding.ASCII.GetString(header[8..12]);
        if (!header[8..12].SequenceEqual(AiffMarker))
        {
            if (string.Equals(formType, "AIFC", StringComparison.Ordinal))
                throw new NotSupportedException("AIFF-C/AIFC wird für DR-Tagging derzeit nicht unterstützt; unterstützt wird nur unkomprimiertes FORM/AIFF.");

            throw new InvalidDataException("Der FORM-Container ist keine AIFF-Datei.");
        }

        var declaredFormSize = BinaryPrimitives.ReadUInt32BigEndian(header[4..8]);
        if ((long)declaredFormSize + 8 != stream.Length)
            throw new InvalidDataException("Die FORM-Größenangabe entspricht nicht exakt der Dateigröße.");
        ...
        while (stream.Position < stream.Length)
        {
            ...
            if (stream.Length - headerOffset < 8)
                throw new InvalidDataException("Die AIFF-Datei enthält einen abgeschnittenen IFF-Chunk-Header.");

            ReadExactly(stream, chunkHeader);
            var id = Encoding.ASCII.GetString(chunkHeader[..4]);
            var size = BinaryPrimitives.ReadUInt32BigEndian(chunkHeader[4..8]);
            var dataOffset = stream.Position;
            var paddedSize = checked((long)size + (size & 1));
            var totalLength = checked(8L + paddedSize);

            if (dataOffset + paddedSize > stream.Length)
                throw new InvalidDataException($"Der IFF-Chunk '{id}' reicht über das Dateiende hinaus.");
            ...
            var isId3 = string.Equals(id, "ID3 ", StringComparison.Ordinal);
            if (isId3)
            {
                if (id3Tag is not null)
                    throw new NotSupportedException("AIFF-Dateien mit mehreren ID3v2-Chunks werden aus Sicherheitsgründen nicht verändert.");
            ...
            if ((size & 1) != 0)
            {
                ...
                if (pad < 0)
                    throw new EndOfStreamException("Der IFF-Chunk besitzt kein erforderliches Padding-Byte.");
            }

            chunks.Add(new IffChunk(id, headerOffset, dataOffset, size, totalLength, isId3));
        }

        if (!gotFmt)
            throw new InvalidDataException("Die AIFF-Datei enthält keinen 'COMM'-Chunk.");
        if (!gotData)
            throw new InvalidDataException("Die AIFF-Datei enthält keinen 'SSND'-Chunk.");

        return new ParsedAiff(chunks, id3Tag);
    }
    ...
    private static ParsedId3Tag ParseId3Payload(byte[] payload)
    {
        if (payload.Length < Id3HeaderLength || !payload.AsSpan(0, 3).SequenceEqual(Id3Marker))
            throw new InvalidDataException("Der AIFF-ID3-Chunk enthält keinen gültigen ID3v2-Header.");
        ...
        if (version is not 3 and not 4)
            throw new NotSupportedException($"ID3v2.{version} wird für AIFF-DR-Tags nicht unterstützt. Unterstützt werden nur ID3v2.3 und ID3v2.4.");
        if (revision != 0)
        ...
        var declaredLength = checked(Id3HeaderLength + bodyLength);
        if (declaredLength > payload.Length)
            throw new InvalidDataException("Die ID3v2-Größenangabe reicht über die Größe des AIFF-ID3-Chunks hinaus.");

        var trailingChunkPadding = payload.AsSpan(declaredLength).ToArray();
        if (trailingChunkPadding.Any(value => value != 0))
        ...
        return new ParsedId3Tag(version, bodyLength, frames, trailingChunkPadding);
    }
    ...
    private static void ReadExactly(Stream stream, Span<byte> destination)
    {
        ...
        while (total < destination.Length)
        {
            ...
            if (read == 0)
                throw new EndOfStreamException("Unerwartetes Dateiende beim Lesen der AIFF-Datei.");
            total += read;
        }
    }
    ...
    private sealed record ParsedAiff(IReadOnlyList<IffChunk> Chunks, ParsedId3Tag? Id3Tag);

    private sealed record IffChunk(
        string Id,
        long HeaderOffset,
        long DataOffset,
        uint DataLength,
    ...
    private sealed record ParsedId3Tag(
        ...
        byte[] TrailingChunkPadding)
    {
        public static ParsedId3Tag WithoutId3() => new(null, 0, Array.Empty<ParsedFrame>(), Array.Empty<byte>());
    }
